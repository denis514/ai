...
- [ ] **P1 — Демо-шаблоны «Собрать в Agent Builder»** (мост learn→do, активация).
  ...
  Шаги: индекс узел→{templateId,demoStart} → кнопка в DetailPanel → связать 13 готовых →
...
- [ ] Фаза 0 — каркас дашборда (раздел «Обзор» по умолчанию, сетка виджетов).
- [ ] Фаза 1 — ядро ученика: Продолжить + Прогресс + На повторение.
- [ ] Фазы 2–6 — вовлечение, мост в Builder, углублённые разделы, гость, барьеры.
