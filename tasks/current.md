...
Осталось из той же ревизии (open):
- [ ] Один экспорт/сброс для профиля и кабинета (включая конструктор); тексты:
  тизер «Pro: синхронизация», подсказка про имя/тему у гостя; вкладка
  «Автозапуски» у гостя без подсказки; вход из интро не отмечает интро;
  `/account` недостижим для гостя из интерфейса; превью курса всегда 0 %
  (`TutorialDetail.jsx:47` ищет по id узла); мёртвый гейт курсов в
  `TutorialModal`/`App.jsx`; `syncNodeProgress` на каждый рендер (`counts`).
- [ ] Замечания судьи (низкий риск): `skipDirtyRef` на пустом холсте;
  задокументировать компромисс «невольный выход → вход другого»; update-путь
  `saveWorkflow` без транзакции (delete-then-insert) — RPC; `syncReady` при
  полном провале pull ставится без попытки переезда.
