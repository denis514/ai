...
- [ ] **Защита кошелька**: потолок запусков/токенов на пользователя в сутки
  (автозапуск идёт без человека → риск трат). ⭐ важно для безопасности.
- [ ] **Журнал автозапусков в UI**: история срабатываний + ошибки прямо в Builder
  (сейчас только через SQL).
- [ ] **Триггер «вебхук»**: запуск схемы по внешнему событию.
- [ ] **Коннекторы**: email-выход, календарь.
- [ ] Code-exec / MCP — реальное исполнение (остаток Фазы 4).
