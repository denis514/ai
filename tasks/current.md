...
⚠️ **Деплой-долг:** `supabase functions deploy builder-execute` и
`supabase functions deploy builder-generate` (оба изменены). После деплоя —
один живой запуск схемы с веб-поиском на проде (судья: убедиться, что поиск
срабатывает на Sonnet 5 с выключенным размышлением).
