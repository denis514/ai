...
## 🧭 Где мы сейчас (2026-05-28): Builder — edge/connection итерация
...
**Группа A — тех-долг (сделать ДО новых фич, дёшево):**
- [ ] A1. `historyBridge.push` → `useEffect` + cleanup (убрать мутацию синглтона). [S]
- [ ] A2. Сериализовать `sourceHandle`/`targetHandle` рёбер — иначе loose-геометрия теряется при save/load. [S/M, проверить схему БД]
- [ ] A3. `isValidConnection`: нормализовать направление по origin / перепроверить цикл после swap. [S]

**Группа B — полировка соединений (UX-агент):**
- [ ] B1. Не открывать панель действий поверх хэндлов выбранного узла (конфликт с loose-mode). [S]
- [ ] B2. Пульсация только одного канонического порта (bottom out / top in). [S]
- [ ] B3. Edge selectable + Delete с клавиатуры + grace-таймаут на кнопку разъединения. [S]
- [ ] B4. Локализовать EN-строки хэндлов + aria на кнопке разъединения (§14a). [S]

**Группа C — ценность для Beta (flow-агент):**
- [ ] C1. **H4 pre-run валидация** — предупредить «нет старта/выхода/висящие связи» перед реальным запуском (защита токенов). [S]
- [ ] C2. **Markdown-рендер результата** + «что дальше» (сейчас сырой текст). Quick win. [S/M]
