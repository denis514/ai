...
- 🟡 **Вес первой загрузки.** Сделано 18-19.08: (1) контент грузится для одного
  языка, а не двух; (2) тексты туториалов (~780 KB gzip на язык) больше не едут
  при входе — только индекс заголовков, тело по требованию. При входе на `/ru`
  теперь: core-узлы + titles + библиотека промптов (+ sys/commerce в фоне).
  Сделано 19.08: (3) по требованию грузятся личный кабинет (тянул за собой сервисы
  конструктора), окно входа, справка, панель профиля, палитра команд — входной
  JS 185.8 → **180.6 KB gzip** (raw 615 → 581).
  Сделано 19.08: (4) шаги туториалов больше не дублируются в `tutorials.js`
  (124 → 72.6 KB): id и порядок шагов живут в локалях, в структуре осталось
  `stepCount`. Входной JS **179.2 KB gzip** — критический порог снят, осталось
  предупреждение. ⚠️ Ожидание не подтвердилось: раскрытая оценка «~8 KB gzip»
  считалась по сырому размеру, а повторяющиеся `{ id: '…' }` жмутся почти в ноль —
  реальный выигрыш 1.5 KB gzip (17 KB raw, меньше работы парсеру). Настоящая
  ценность правки в другом: id шагов больше не лежат в четырёх местах сразу.
  Осталось: `mindmapData.js` 85 KB — это структура карты, нужна для первого кадра.
  По CSS порог снят: «50.8 KB» складывали стили конструктора и стайлгайда, которые
  на карте не грузятся. Реальная первая загрузка — **28.6 KB**, счётчик в
  `scripts/perf-audit.mjs` исправлен (строка «CSS первой загрузки» по `index.html`).
  ⚠️ `npm run audit:perf` меряет РАЗМЕРЫ ЧАНКОВ, а не то, что реально
  скачивается при загрузке страницы, поэтому выигрыш от (1) и (2) в отчёте не
  виден. Проверять руками через вкладку «Сеть» или `performance.getEntriesByType`.
...
### 🧭 Сессия 2026-08-19 (продолжение 6) — шаги туториалов из структуры в локали
- ✅ `src/data/tutorials.js` 124 → 72.6 KB: массивы `steps:[{id}]` заменены на
  `stepCount`. Идентификаторы и порядок шагов теперь только в локалях (ключи
  объекта `steps`) — проверено, что во всех трёх языках порядок совпадал с
  прежней структурой для всех 261 туториала.
- ✅ `buildById` в `useTutorial.js` строит шаги из тела туториала; пока тело не
  загружено, шагов нет — ровно это и показывает флаг `bodyReady`, который UI уже
  умел обрабатывать.
- ✅ `sync-whats-new.mjs` восстанавливает прежнюю форму `[{id}]` из локали для
  хеша — иначе смена формата пометила бы все 261 туториал как «обновлён».
  Проверено: после правки скрипт говорит «контент не изменился».
- 📉 Входной JS 180.6 → **179.2 KB gzip** (raw 580.6 → 563.3). Критических
  пунктов в перф-отчёте больше нет.
- ⚠️ Честно: ожидал ~8 KB gzip, вышло 1.5. Оценка была по сырому размеру, а
  однотипные `{ id: '…' }` сжимаются почти в ноль. Правка всё равно оправдана —
  ушло дублирование id шагов между структурой и тремя локалями.
- Проверено вживую: курс открывается, 6 шагов в списке, переход «Готово-дальше»
  работает; в списке курсов счётчики шагов настоящие; на узле карты кнопка курса
  показывает «6 шагов».

### 🧭 Сессия 2026-08-19 (продолжение 5) — вес первой загрузки
- ✅ Ленивыми стали: личный кабинет (вместе с сервисами конструктора — списком схем
  и расписаниями), окно входа, справка, панель профиля, палитра команд.
  Проверено вживую: карта рисуется, Cmd+K открывает поиск, кнопка профиля открывает
  вход, `/ru/account` рендерится и подгружает свой кусок по требованию.
- ✅ Входной JS 185.8 → 180.6 KB gzip. Состав чанка мерил по sourcemap, а не на глаз.
- 🔍 Счётчик CSS в аудите врал: складывал стили карты (28.6) со стилями конструктора
  (19.7) и стайлгайда (2.5). Исправлено: «CSS первой загрузки» берётся из `index.html`.
- ✅ **Метки «новое» гаснут через месяц** (решение владельца, 2026-08-19): срок
  жизни записи вынесен в `WHATS_NEW_TTL_DAYS` (30 дней) + `isFresh()` в
  `useWhatsNew.js`. Раньше правил было три: у меток срока не было вовсе, панель
  «Обновления» и виджет кабинета резали по своим 60 дням. Теперь правило одно,
  тексты «Последние 60 дней» заменены на «За последний месяц» (ru/en/fi).
- ✅ **Починен архив обновлений** (нашлось при проверке): он молча выбрасывал
  записи туториалов уже ПОСЛЕ нарезки на страницы, поэтому первые страницы
  выглядели пустыми, а счётчик обещал 366 записей. Теперь отсев до нарезки,
  туториалы показываются с названиями и открываются по клику, подзаголовок
  честно говорит «Вся история» (357 записей: 9 указывают на исчезнувшие узлы).

### 🧭 Сессия 2026-08-19 (продолжение 4) — схемы под непокрытые темы карты
- ✅ **6 новых схем** (33 → 39), каждая под конкретный узел, у которого не было демо:
  «Ответ по вашему документу» (файлы + цитаты, условие «нет в документе»),
  «Разбор кода» (агент-программист, условие «критично»), «Из пожеланий в требования»
  (умное условие: выдать требования или задать вопросы), «Подбор товара покупателю»
  (каталог файлом, честное «нет подходящего»), «Персональное предложение»
  (память + цикл-редактор), «Что мешает покупать» (зрение + аналитик + умное условие).
- Все шесть по правилам: результат на экране (без подключений), ≥2 переменные в
  «Старте», у каждой есть логика, тексты ru/en/fi.
- ✅ **Покрытие узлов 25 → 30**; `cap-files` и `uc-ai-rag-launch` получили
  собственную схему вместо общей с `cap-code-exec`/`product-qa`.
- Сделано 6 схем вместо плановых «~10»: остальные узлы из §2 плана уже покрыты,
  а добавлять демо ради счёта — против правила честности в том же документе.

### 🧭 Сессия 2026-08-19 (продолжение 3) — каталог схем по роду занятий
- ✅ **Категории галереи переписаны под деятельность** вместо устройства схемы:
  Поддержка клиентов (8), Контент и блогинг (6), Продажи и заказы (5),
  Исследования (4), Дизайн (3), Люди и обучение (3), Аналитика и отчёты (2),
  Разработка (2). Было: everyday (13 из 33 в одной куче), logic, marketing,
  design, research, data — деление по внутреннему устройству, а не по задаче.
- ✅ **Чипы-фильтры в галерее** с числом схем у каждой категории; пустые категории
  не показываются. Чипы — канонические `.chip` из стайлгайда, свой стиль не
  вводили (в CSS добавлена только раскладка ряда).
- ✅ `lint:builder` проверяет: у каждой схемы категория из реестра и у каждой
  категории есть перевод во всех трёх языках.
- 🎨 **Починен общий чип в тёмной теме**: `.chip.is-active` был жёстко `#1a1a1a`
  и в тёмной теме сливался с фоном — активный фильтр читался только по цвету
  текста. Переведён на токены (`--text` на `--bg`), в светлой теме вид прежний.
  Правка общая — касается и фильтров карты, поэтому сделана в `App.css`,
  а не локально в билдере.

### 🧭 Сессия 2026-08-19 (продолжение 2) — первый результат для новичка
Прошёл путь гостем на проде: кнопка из карты → готовая схема → «Запуск» → **голая
форма входа без единого слова почему**. При этом «Тестовый запуск» работает для
гостя без входа и бесплатно, но новичок его не замечает.
- ✅ **Окно-объяснение** (собрано из существующих классов модалки, свой стиль не
  вводили). Два случая: приход из карты («блоки соединены, задание заполнено,
  нажмите тестовый запуск — бесплатно и без входа», один раз на устройство) и
  попытка настоящего запуска без ключа («понадобятся вход и ваш ключ Claude,
  прогон тратит ваши токены, а не наши» + «Сначала бесплатный тест»).
- ✅ **Связка `cap-search` перевешена** с `topic-digest` (заканчивался отправкой в
  Telegram — без подключённого бота результат не увидеть) на `cited-answer`
  (ответ со ссылками на экране). Проверено: из 25 связок остальные 24 и так
  показывают результат на экране.
- Проверено вживую гостем: подсказка → «Запуск» → объяснение → «Сначала бесплатный
  тест» → прогон идёт по шагам без входа.

### 🧭 Сессия 2026-08-19 (продолжение) — мост «узнал → собрал»
- ✅ **Кнопка «Собрать в конструкторе»** в панели узла: открывает Agent Builder с
  готовой схемой по теме узла. Индекс `src/data/nodeTemplates.js` — 25 узлов на
  17 шаблонов (способности агента, роли, паттерны схем, сценарии). Кнопка есть
  только у узлов из индекса — информативным узлам демо не выдаём.
- ✅ Маршрут `/<lang>/builder/<templateId>` (роутер уже поддерживал `{type,id}`),
  `BuilderApp` принимает `initialTemplateId` и грузит шаблон один раз.
- ✅ `npm run lint:builder` теперь падает, если запись индекса ссылается на
  несуществующий шаблон или на удалённый узел карты.
- 🐞 **Найдено и починено попутно (серьёзное)**: при заходе по прямой ссылке на
  узел из секции `sys`/`commerce` текст узла мог не появиться ВООБЩЕ — на экране
  оставались сырые ключи вида `nodes.uc-ai-support-tier1.title`. Причина:
  `loadLocaleContent` присваивал `STRINGS[locale].nodes` целиком и затирал
  секцию, которую параллельно успел догрузить `getNode`. Теперь мёржит.
  Баг был и до правок этой сессии, просто маскировался англоязычным запасом.

### 🧭 Сессия 2026-08-19 — тексты курсов по требованию
- ✅ **Индекс заголовков** `src/locales/<loc>/tutorials/titles.json` (261 запись,
  50-70 KB) вместо ~4 MB полных текстов при входе. Генератор
  `scripts/build-tutorial-index.mjs` привязан к `npm prebuild` и `npm run sync`,
  плюс `npm run lint:tutindex` не даст индексу разойтись с текстами.
- ✅ **Тело курса грузится в момент показа** — `ensureTutorialBody()` по полю
  `audience`. Важно: вызывать только на экранах, где текст показывают; в списках
  это утянет все три чанка сразу (первая версия так и делала, поймано на тесте).
- ✅ Скелетон вместо пустых секций, пока тело едет (`tut.bodyReady`).
- 🐞 **Аудит производительности мерил не тот чанк**: искал чанк с именем `index`,
  а после появления `titles`-чанков их стало несколько. Теперь входной чанк
  берётся из `dist/index.html`; vendor считается до него, как и раньше.
- 📌 ADR-запись в `docs/architecture.md`.

### 🧭 Сессия 2026-08-18 (продолжение) — первая загрузка похудела вдвое
- ✅ **Проверен прод снаружи**: все 8 edge-функций отвечают, миграции 006 и 007
  применены (таблицы `builder_mcp_servers` и `builder_webhooks` существуют),
  RLS отдаёт анониму пустоту по всем таблицам. Владелец прогнал реальный агент:
  веб-поиск → ответ → доставка в Telegram, 8535 токенов. Деплой-долг закрыт.
- ✅ **Контент грузился на двух языках вместо одного** (commit ниже). Две причины:
  1) `LocaleContext` всегда тянул запасной `en` рядом с текущим языком, хотя все
  три локали заполнены одинаково; 2) на первом визите приложение стартовало на
  `DEFAULT_LOCALE` и начинало качать его контент ДО того, как определится язык по
  IP, а потом качало второй комплект. Теперь: контент ждёт, пока язык устоится
  (страховка 800 мс), запасной язык подтягивается только по факту промаха.
- 🐞 **Попутно найдено**: `_loaded` помечался ДО загрузки, поэтому «контент
  загружен» было правдой уже во время запроса — из-за этого любая проверка
  готовности врала. Добавлен отдельный `_ready`.
- 🧹 Удалён чужой отладочный хук `window.__contentLoads` из `strings.js`
  (оставался незакоммиченным в рабочей копии, в `main` его не было).
