...
⚠️ **Деплой-долг (ДО слияния в main, по порядку):**
1. SQL: хвост `docs/supabase-setup.sql` — `ALTER TABLE learning_progress ADD
   COLUMN IF NOT EXISTS started_at` (идемпотентно). Без колонки прогресс курсов
   перестанет зеркалиться (ничего не разрушается).
2. `supabase functions deploy account-delete` — до этого кнопка «Удалить
   аккаунт» показывает ошибку (данные не трогает).
