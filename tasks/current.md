...
- [ ] Шаг с веб-поиском стоит ~50k токенов (результаты поиска — тяжёлый вход;
  модель делает до 8 подзапросов): показать человеку ориентир стоимости до
  запуска и подумать о `max_uses` 5 / компактном режиме.
