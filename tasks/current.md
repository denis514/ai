...
**За владельцем (панели):**
- [ ] Supabase → Auth → URL Configuration: Site URL = `https://www.105-atlas.app`,
  в Redirect URLs добавить `https://www.105-atlas.app/**` (и `https://105-atlas.app/**`).
  Без этого вход (Google/ссылка на почту) с нового домена не вернётся на сайт.
- [ ] GA4: адрес потока данных → новый домен; снять галочку «Изменения в
  истории браузера» в «Улучшенной статистике» (двойной счёт).
- [ ] Google Search Console: подтвердить домен, подать sitemap, «Change of address».
- [ ] Почтовый ящик `privacy@105-atlas.app` (указан в GDPR-блоке кабинета) —
  завести пересылку, иначе письма в никуда.
