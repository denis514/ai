...
- **«Войти через Apple» — на паузе (решение владельца, 2026-08-22).**
  Не покупаем Apple Developer аккаунт и не настраиваем провайдера; к вопросу не
  возвращаемся, пока владелец сам не поднимет. Код готов и в проде, скрыт флагом
  `APPLE_SIGNIN_ENABLED = false` в `src/components/AuthModal.jsx`; включение —
  по `docs/auth-apple-setup.md` (аккаунт $99/год → провайдер в Apple и Supabase →
  флаг в `true` → деплой). (Изначально отложено 2026-06-11.)

---

## ✨ Автосборка «опиши задачу → схема» + новый первый экран (2026-08-23)

Решение основателя (07-decisions.md): строка «опиши задачу» с печатающимися
примерами вместо кнопки «Старт»; 3 спонсорские автосборки в день на гостя
(ключ `ATLAS_SPONSOR_KEY`, установлен); со своим ключом — без дневного лимита.
Формулировки строго разделяют «автосборки» (лимит) и ручную сборку (безлимит).

Сделано (барьер: BLOCK по кошельку → все 3 блокера исправлены → PASS):
- edge-функция `builder-generate`: Haiku, строгая валидация кубиков, лимиты
  3/день клиент + 6/день IP + месячный потолок; fail-closed на ошибках БД,
  резервирующая запись ДО вызова Claude (гонка закрыта), доверенный конец XFF.
- миграция `012_builder_generations.sql` (журнал, IP только хешем, RLS service-only).
- клиент: GenerateHero (машинка-примеры, prefers-reduced-motion), generateService,
  авто-раскладка схемы, вписывание в холст, автосохранение как новая схема.
- Тексты ru/en/fi, включая честный счётчик и отдельный текст месячного потолка.

**Выкачено и проверено 2026-08-23:** миграция 012 применена, функция
опубликована, ветка слита. Живой гостевой тест на проде: 3 автосборки прошли
(схемы осмысленные, включая доставку в Telegram по запросу; расход ~2-3¢),
4-я остановлена 429 daily_limit ДО обращения к Claude, счётчик 2→1→0. ✅

Замечания судьи (backlog, низкий риск): атомарная RPC для лимитов; учёт токенов
при not_a_task/502; клиентский счётчик после ошибок; подпись при падении
расшифровки ключа; двойной вызов clientId().

---

## 🔎 Инициатива: умный поиск обычными словами (2026-08-22)

Стратегия: `docs/smart-search-plan.md`, бэклог №5a.

**Этап 1 — ГОТОВ ✅ (2026-08-22, барьер качества PASS):**
- Движок `src/services/searchEngine.js`: MiniSearch + Snowball-стеммеры
  ru/en/fi + стоп-слова + префикс + опечатки + веса полей + синонимы
  (`src/data/searchSynonyms.js`, сопоставление по основам, совокупный вариант)
  + мягкий ИЛИ-фолбэк. Ленивый чанк: вход 177 KB gz (бюджет 180 соблюдён).
- Заменил ОБА механизма: фильтр карты (набор совпавших id) и палитру
  (единый индекс: узлы с телом + курсы + промпты).
- Метрика: золотой набор 46 запросов (`tests/search-golden.json`),
  `node scripts/search-eval.mjs` — **100% против 13% у старой подстроки**;
  eval импортирует реальный движок (без дубля логики).
- Живые проверки: склонение, опечатка, синоним, пустое состояние, палитра.
- Замечание судьи «запрос из одних стоп-слов → пусто без фолбэка» — принято
  как честное поведение (open, низкий приоритет).

**Этап 2 (open):** семантика в Supabase (pgvector + гибрид RRF,
мультиязычные эмбеддинги) — по плану.

---

## 🧑‍🎨 Профиль и кабинет: «плашка — взгляд, кабинет — дом» (2026-08-22)

По плану владельца, фазы 1-3 выполнены:
- Ф1: язык в кабинете — единый `LanguageSwitcher`; мёртвое read-only поле имени убрано.
- Ф2: плашка профиля — короткая карточка: имя (тап → кабинет), карточка гостя,
  прогресс разборов + «Продолжить: <активный>», счётчики карты, «Выйти».
  Убраны: Pro-тизер, списки «В процессе»/«Пройдено» (живут в кабинете),
  блок «Данные» гостя (переехал в настройки кабинета, добавлен «Импорт»).
- Ф3: кабинет — единственное место настроек: редактируемое имя (вошедший →
  профиль, гость → браузер), новый общий `ThemeSwitcher` (парный к
  LanguageSwitcher, те же классы, зарегистрирован в стайлгайде), порядок секций
  Профиль → Сессия → Данные → Куки → GDPR → Опасная зона, Pro — одной строкой.

Проверено в браузере (десктоп + 375px): панель короткая, имя сохраняется,
тема переключается, порядок верный, перелива нет.

Продолжение (2026-08-22): **стандарт шапки зафиксирован** (стайлгайд, секция
«Стандарт шапки»): слева — «назад/контекст», справа/в ряду — ThemeSwitcher +
LanguageSwitcher, именно в этом порядке, общими компонентами. Применён на всех
трёх поверхностях: шапка Atlas мигрирована с самодельного блока темы на
ThemeSwitcher, в кабинет добавлен правый кластер, в Builder — тема рядом с
языком. Ряд «Язык и тема» из настроек кабинета убран (дубль шапки).
Долг (мелкий): осиротевшие стили `.account-lang-btn` и имя-редактирования
панели вычистить из App.css.

---

## ⏸️ Supabase Custom Domain — отложено (решение владельца, 2026-08-22)

Окно Google-входа показывает технический адрес проекта
(`uzqrwlpijnmxjzyrxkur.supabase.co`). Заголовок окна лечится бесплатно
брендингом OAuth в Google Console (App name «105 Atlas» — настроено
владельцем). Полная замена адреса на свой (`api.105-atlas.app`) требует
Supabase Pro (~$25/мес) + Custom Domain (~$10/мес) — отложено до
монетизации. Project ID не меняется в принципе.

---

## ⚖️ GDPR-ревизия (2026-08-22)

Аудит по запросу владельца: политика была от 17 мая и отстала от продукта.
Что проверено и сходится: правовые основания по таблицам, Consent Mode v2
(по умолчанию запрещено, GA грузится только после согласия — проверено живьём),
anonymize_ip, рекламные сигналы выключены, галочки согласия и 16+ при
регистрации существуют в коде, удаление аккаунта настоящее, выгрузка полная,
право на жалобу (tietosuoja.fi) указано.

Исправлено:
- Плашка cookies: ложное «не передаются третьим лицам» заменено честным текстом
  + ссылка на политику (3 языка).
- Политика (3 языка): дата 22.08.2026; контакт privacy@105-atlas.app; секция
  «Agent Builder» (ключи шифруются, задачи уходят в Anthropic США под ключом
  пользователя, SCC); обработчики дополнены (Anthropic, Vercel Web Analytics,
  ImprovMX, country.is); поправлено «данные не покидают ЕЭЗ»; сроки хранения
  для схем/ключей; localStorage гостя описан.
- ToS: контакт → privacy@.

**За владельцем (одно действие):** GA4 → Администратор → Настройки данных →
Хранение данных → поставить **14 месяцев** (по умолчанию 2, а политика обещает 14).

⚠️ Дисклеймер в docs: это добросовестное соответствие, не юридическая гарантия;
при росте до платного продукта — консультация юриста по ePrivacy/DPIA.

---

## 🌐 Домен 105-atlas.app подключён (2026-08-22)

Боевой адрес: `https://www.105-atlas.app` (голый домен и vercel.app —
редиректы). В коде/статике адрес заменён: canonical/OG, sitemap, robots,
запасные origin (share, meta), e-mail приватности → `privacy@105-atlas.app`,
playwright baseURL, README. Динамические canonical/hreflang сами берут
`location.origin`.

**Выполнено владельцем 2026-08-22 (панели):**
- [x] Supabase Auth URL Configuration: Site URL + Redirect URLs (`/**` для www
  и голого домена). ⚠️ Живой вход с нового домена ещё не проверен — проверить
  при следующем визите.
- [x] GA4: адрес потока → новый домен; галочка «история браузера» снята
  (двойной счёт закрыт, наш `page_view` остаётся).
- [x] Search Console: домен подтверждён (TXT в Vercel DNS), sitemap подан и
  получен. «Change of address» не нужен — старый адрес в GSC не жил, 308 всё
  переклеит.
- [x] Почта `privacy@105-atlas.app` — пересылка через ImprovMX на личный ящик
  владельца (MX ×2 + SPF в Vercel DNS, проверено снаружи 2026-08-22). Ответы
  уходят с личного адреса — для GDPR-переписки допустимо; отправка «от имени»
  privacy@ не настроена (не требуется).

**Внешняя проверка 2026-08-22 (снаружи, живой заход):** 308 со старого адреса
и с голого домена (включая глубокие ссылки), sitemap/robots/canonical/OG — новый
домен, Vercel Analytics шлёт просмотры, GA ждёт согласия (Consent Mode). ✅

## ⏸️ Аналитика — пауза: условие снятия наступило (домен подключён 2026-08-22)

Решение владельца: сайт опубликован, но это тестовая версия, на пользователях её
не проверяют. Поэтому **аналитику не тестируем и отчёты не собираем**. Сбор
событий настроен и работает — трогать его не нужно, просто не делаем выводов по
этим данным. Возврат — при переходе на настоящий домен.
Подробности и чек-лист возврата: `docs/analytics-events.md`.
...
## 📈 Vercel Web Analytics подключён (2026-08-22)

По запросу владельца: пакет `@vercel/analytics`, `inject()` в `src/main.jsx`
(мы на Vite — сниппет Vercel для Next.js не подходит). Без cookies, счётчик
включён на панели Vercel. Пауза «выводов по данным» действует и здесь.
Запись: `docs/architecture.md` (2026-08-22).

---

## 🗣 Словарь интерфейса: простой язык (2026-08-22)

Решение владельца (07-decisions.md, отменяет решение 19.08): «workflow» уходит
из интерфейса. Словарь: разборы Atlas = «разбор» / «guide» / «opas»; схемы
конструктора = «схема» / «automation» / «automaatio»; «курс» также заменён на
«разбор» для единообразия (одно слово на поверхность). ~50-70 строк на локаль
в `ui.json` + запасные тексты в коде. Термин остаётся в содержимом (узлы об AI
workflows как концепте, тексты разборов) — контентная волна отдельно (open).
P0 №1 бэклога закрыт.
...
сессии). Корень проблем: браузер — источник правды, облако — зеркало, но
«развод» (выход, смена человека, удаление) не был продуман.

Сделано (барьер качества: PASS, ветка `claude/project-review-4d60a3`):
- [x] `syncService.js`: несуществующая `user_id` → `userId` (перенос отметок тем
  падал при входе); pull умеет `isStale()`; `resetHydration()`.
- [x] `services/localData.js` — метка владельца локальных данных
  (`atlas:local-owner:v1`) + единый список ключей прогресса. Правила в AuthContext:
  вход при чужом владельце → очистка до pull; владелец ставится только после
  успешного pull; выход по кнопке → досохранить → очистить; невольный разрыв
  сессии → не чистить.
- [x] Конструктор: перед выходом досохраняет холст; запоздавший ответ сохранения
  после выхода игнорируется; грязный холст при обрыве сессии не стирается.
- [x] Удаление аккаунта — через edge-функцию `account-delete`
  (`auth.admin.deleteUser`, каскад по всем таблицам). Раньше удалялась только
  строка `profiles`, данные и учётка оставались. Ошибка показывается в блоке.
- [x] `useActivityLog`/`useUserIdentity` перечитывают после очистки; импорт в
  профиле сразу обновляет экран.

- [x] Схемы гостя переезжают в аккаунт при входе (`migrateLocalToCloud`
  подключён в `AuthContext.runSync`, замок на вкладки, карта id для черновика;
  автосохранение и досохранение ждут `syncReady`). Кабинет показывает гостю его
  местные схемы; подсказка переписана честно (3 языка).
- [x] Серия дней и дата начала курса — в облако: `activity_log` сливается при
  входе, `learning_progress.started_at` (новая колонка). Право писать в облако
  теперь выдаётся по каждой таблице отдельно.

✅ Выкачено 2026-08-22: колонка `started_at` добавлена в проде, функция
`account-delete` опубликована, ветка слита в `main` (`0c81299`).
✅ Живая проверка на проде (владелец, 2026-08-22): вход через Google →
удаление аккаунта → всё стёрто, браузер чист → повторный вход тем же адресом →
чистая история. Ключевой сценарий ревизии подтверждён.

- [x] Косметика и тупики (2026-08-21, барьер PASS): один модуль выгрузки/
  импорта/сброса `services/dataExport.js` для профиля и кабинета (включая схемы
  конструктора, активность, уровень; у вошедшего — плюс облако); кнопка
  «Сбросить всё» гостю в кабинете; `/account` доступен гостю из профиля;
  вкладка «Автозапуски» гостю объясняет и предлагает войти; вход из интро
  отмечает интро пройденным (плашка согласия больше не глохнет); превью курса
  показывало 0 % (читало по id узла); мёртвый гейт курсов удалён
  (`suspended`/`onRequestAuth`/`tutorialAwaitingAuth`); `counts` мемоизирован —
  синк тем не дёргается на каждый рендер; тексты: тизер Pro без «синхронизации»,
  подсказка гостю в настройках, текст сброса = реальный список. Модель данных
  задокументирована в `docs/architecture.md` (2026-08-21).

Осталось (низкий риск, backlog):
- [ ] `skipDirtyRef` на пустом холсте; update-путь `saveWorkflow` без
  транзакции (delete-then-insert) — RPC; `syncReady` при полном провале pull
  ставится без попытки переезда; выгрузка вошедшего при недоступном облаке
  молча помечается `browser` — показать тост.

---

## 🧭 Правая панель с вкладками (2026-08-23)

- [x] **Объединение** ✅: плавающее окно консоли удалено (`ConsoleWindow.jsx`,
  его состояние и ресайз); тест и запуск пишут в «Журнал» панели; ⤢ растягивает
  панель до 60 % (вместо второго окна); «Код схемы» — экран внутри панели с
  «Назад»; вкладки — иконки, подпись только у активной (все 4 видны на 340px);
  после теста остаёмся в «Журнале», после настоящего запуска — «Детали».
- [x] **Журнал — вертикальный таймлайн** ✅: карточка на шаг (имя блока
  человеческим словом + что произошло, время справа), слева линия с
  галочкой / точкой / крестом / пульсом; обычный шрифт вместо моно.

- [x] **Заход A** ✅: одна правая панель, переключатель «Детали · Журнал ·
  Автозапуски · Подключения»; консоль и окно ключей переехали внутрь
  (журнал можно «Развернуть» в большое окно; «Код схемы» — в «⋯»);
  автопереключение: блок → Детали, запуск → Журнал, готово → Детали;
  значки: номер шага на «Деталях», точка при работе на «Журнале», «!» на
  «Подключениях» без ключа; панель открыта при любой схеме; меню «⋯» — 4 пункта.
- [x] **Заход B** ✅: чек-лист внутри «Деталей» (вертикально, у текущего
  шага кнопка действия; «Всё готово» с закрытием), плавающая полоса с холста
  убрана; обратный билет из «Подключений»: кнопка «← К блоку» и
  автовозврат к блоку после подключения; панель 340px под 4 вкладки.

## 🧭 Дорога до первого успеха (2026-08-23) — ГОТОВО ✅

- [x] Ключ Claude: в окне «Подключения» — инструкция по 3 шагам (консоль →
  баланс от $5 с ценой запуска → создать ключ) + ссылки на консоль и пополнение.
- [x] Telegram: 4 шага (BotFather → токен → /start → @userinfobot) + ссылка
  на официальную инструкцию. Почта: 3 шага + ссылки на ключи и домен Resend.
- [x] Одно имя везде: окно «Подключения» (было: Ключи API / Коннекторы / Мои
  ключи / Ключи / «замок сверху»); справка переписана ru/en/fi.
- [x] Расписание — по местному времени (перевод в UTC на клиенте; списки
  расписаний и история — тоже по местному). Подсказка «Москва = +3» убрана.

## 🧭 Упрощение интерфейса конструктора (2026-08-23) — план из 3 заходов

Экспертный разбор (в чате): после сборки новичок видел 48 активных элементов,
10 кнопок в шапке, две почти одинаковые кнопки запуска, панели открывались
сами, «следующего шага» не было. Принцип: один экран — одно действие.

- [x] **Заход 1** ✅: шапка с 10 кнопок до 4 (плашка+тема+язык · имя схемы ·
  «⋯» · одна кнопка «Запустить»); тестовый запуск — внутри окна перед
  запуском (окно показывается всегда, с ценой); меню «⋯»: Подключения,
  Журнал, Автозапуски, Код, Очистить; на пустом холсте справа пусто;
  убраны кнопки «показать детали» и «консоль» из шапки. Кнопка палитры
  остаётся при схеме — до появления «+ Добавить блок» (заход 2).
- [x] **Заход 2** ✅: полоса «Следующий шаг» под шапкой (4 шага с галочками;
  текущий подсвечен; клик ведёт точно туда: к ненастроенному блоку, к
  блоку доставки, к запуску, к расписанию; когда всё ✓ — закрывается);
  палитра не открывается сама, на холсте «+ Добавить блок»; кнопка палитры
  из шапки убрана. Подключение бота/почты — уже в карточке блока.
- [x] **Заход 3** ✅: после запуска правая панель показывает карточку
  «Готово» с результатом и кнопками «Повторять по расписанию» / «Показать
  ход работы» / «Копировать»; масштаб — только проценты и «вписать»
  (±, мини-карта убраны с холста; мини-карта и горячие клавиши — в «⋯»);
  подсказки клавиш — по «?». Тексты ru/en/fi.

---

## ⛔ Аудит ограничений автоматизации (2026-08-23) — решения ждут владельца

Четыре эксперта + проверка: `docs/agent-builder/17-automation-limits-audit-2026-08.md`.
Суть: продукт — конструктор цепочек агентов с доставкой, не автоматизатор
событий; из 12 типичных задач целиком работает одна. Три дыры (нет входящих
событий, нет памяти между запусками, нет записи «куда-то») + автосборка не
умеет отказывать + 3 из 4 примеров в строке ввода невыполнимы.

**Дешёвый слой (§6 п.1-6) — ГОТОВ ✅ (2026-08-23, барьер: BLOCK → PASS):**
1. Модель → `claude-sonnet-5` (+ `BUILDER_MODEL` секрет), «размышление»
   выключено — иначе оно съедало `max_tokens` и ломало «условие-агент».
2. Подписи кубиков — по факту (память внутри прогона, файлы только текст,
   4 роли — подсказки, research без открытия страниц, цикл без инструментов).
3. Примеры в строке ввода — 4 выполнимых, все помещаются (ru/en/fi измерено).
4. Автосборка умеет отказывать: `{"error":"unsupported","why"}` + `note`,
   что схема не сделает; клиент показывает словами.
5. Доставка проверяется ДО траты токенов (бот/Resend/адрес/календарь);
   15 ошибок движка + ошибки запуска — на языке интерфейса.
6. Правда про дневной лимит (выключены, включить заново) + уведомление в
   Telegram схемы при сбое автозапуска.

✅ **Выкачено и проверено живьём на проде (2026-08-23):** автосборка
(гость: сборка/отказ/лимит 3; разработчик: без лимита), запуск без
инструментов (8 с, 640 токенов), запуск с веб-поиском (35 с, реальный дайджест).
По ходу найдено и починено (движок): зависание вызова модели → лимит 100 с
и «жнец» мёртвых прогонов (раньше блокировали 10 мин); `max_uses_exceeded` —
Claude 5 ищет параллельно, потолок 3 → 8 + правило «по одному, без повторов»;
диагностика поиска в журнале (код ошибки / число запросов); `web_search_20260209`.
Сборщик: три итерации правил отказа + переспрос после первого отказа.

Открыто после живого прогона:
- [x] Ориентир стоимости (2026-08-23): модель `src/builder/data/nodeCost.js`,
  монетка на дорогих кубиках (поиск/код) в палитре и на холсте, строка
  «Один запуск: ≈ 52 тыс. токенов ≈ $0.13» в окне перед запуском, «В месяц»
  в расписании. Цена возвращена как «≈» по решению владельца.
- [x] Живой ход выполнения (2026-08-23): движок в потоковом режиме, строка
  «думает…» с мигающей точкой, секундами, токенами по ходу и текущим
  поисковым запросом; прогресс обновляет строку на месте. Барьер: PASS.
  Живая проверка на проде 2026-08-23: монетка, «Один запуск: ≈ 52 тыс. ≈ $0.13»,
  строка «думает… · 33 с · ≈ 6 318 токенов · ищу: «Helsinki cafe …»» — всё
  видно; расход с поиском снизился ~55k → ~40k благодаря правилу «по одному».
  Замечание: токены по ходу обновляются только на границах ходов модели
  (число держится, пока идёт поиск) — это свойство потока, не ошибка.
- [ ] `max_uses` 5 / компактный режим поиска — экономия (open).
- [x] Строки хода выполнения переведены (ru/en/fi): все 47 сообщений движка —
  через словарь, включая роли агентов и цикл/ветки (2026-08-23).
...
- [ ] Режим разработчика: статус `unlimited` виден только в ответе сервера;
  подпись под строкой показывается после первой сборки — ок, но в кабинете
  стоит отметить «режим разработчика».

Замечания судьи в бэклог: версии инструментов `web_search_20260209` /
`code_execution_20260521` (отдельной задачей, с пересмотром лимитов).

**Движок (§6 п.7-10) — open:** память между запусками → события «сообщение
боту»/«письмо» → «спроси меня» → запись наружу/черновик автозапуска.

---

## ✅ Сессия 2026-08-31 — карта догнала документацию

docs-watcher (авто-прогоны 24 и 31.08) → обновлены 3 узла в ru/en/fi:
`cc-ide` (JetBrains + сторонние провайдеры), `cc-scheduled-runs` (Routines:
где кнопка, версия Desktop, лимиты), `pl-desktop` (двусторонние сообщения
между сессиями + synced plugins). Метки «обновлено 2026-08-31» в проде,
проверено снаружи. Ревью по `weekly-strategy-2026-08-31.md` не проводилось —
отдельным заходом.

**Подсказка выката (обновлена):** в `main` регулярно пишут авто-сторожа,
поэтому команда теперь с обычным слиянием:
`git pull --no-rebase --no-edit && git merge --no-edit origin/claude/project-review-4d60a3 && git push`
