...
- 🟡 **Вес первой загрузки.** Половина лишнего убрана 2026-08-18: качали контент
  ДВУХ языков вместо одного (~0.9-1 МБ gzip впустую). Осталось: `index`-чанк
  185.8 KB gzip, `App.css` 50.8 KB, и главное — туториалы (~780 KB gzip на язык)
  грузятся целиком при входе, хотя нужны только при открытии курса. Следующий
  шаг: лёгкий индекс заголовков + тело туториала по требованию.
