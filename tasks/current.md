...
## 🔧 Технический аудит (перемерено 2026-08-18)

### Build
| Метрика | Значение | Порог | Статус |
|---------|----------|-------|--------|
| Build time | ~2.2s | — | ✅ |
| JS initial (gzip) | 185.8 KB | 120 / 180 | 🔴 вырос на 47.6 KB с 2026-06-04 |
| JS initial (raw) | 615.4 KB | — | 🔴 |
| CSS всего (gzip) | 50.8 KB | 25 / 40 | 🔴 вырос на 12.3 KB |
...
Отчёт: `tasks/perf-audit-2026-08-18.md`. Замер сделан локально **без ключей
Supabase** — в этой сборке клиент Supabase вырезается tree-shaking'ом, поэтому
строка «Vendor −53.5 KB» в отчёте обманчива: в проде он на месте.
Инициальный JS и CSS перешли критический порог — нужна отдельная задача.
...
- 🔴 **Вес первой загрузки за критическим порогом** (JS 185.8 / CSS 50.8 KB gzip) —
  разобрать `index`-чанк и `App.css`, вынести редкое в ленивые куски.
- ⚠️ 8 неопределённых CSS-переменных в `App.css` (`--text-secondary`, `--bsheet-max`,
  `--side-top`, `--text-3`) — `npm run lint:css`.
...
- [ ] **P1 — Демо-шаблоны «Собрать в Agent Builder»** (мост learn→do, активация).
  Карта узел→шаблон + правила (Старт ВСЕГДА заполнен ru/en/fi; сложность через логику
  условие/цикл/условие-агент; выходы TG/Email/Календарь + задел под MCP/соцсети) +
  рыночный анализ интеграций → `docs/agent-builder/16-node-template-demos.md`.
  Сделано: «Старт» с демо-задачей во всех шаблонах (`70b7849`, `9ba604c`), переменные
  «Старта» (`0a8f6e4`), категории галереи по индустриям (`690e560`), 33 шаблона.
  Осталось: индекс узел→{templateId,demoStart} → кнопка в `DetailPanel` (проверено
  2026-08-18 — её нет) → связать узлы Atlas с шаблонами →
  заполнить «Старт» во всех шаблонах → ~10 новых шаблонов с уровнями → категории галереи
  по деятельности → Фаза B (MCP) → Фаза C (унифиц. постинг соцсетей). НЕ вперёд оплаты.

### 🧭 Сессия 2026-08-18 — возобновление после паузы (уборка автоматизации)
Разработка стояла с 13 июня (последний содержательный commit `eb65972`); два месяца
в main шли только авто-коммиты сторожей.
- ✅ **Токен GitHub убран из адреса репозитория** в связку ключей macOS. Сам токен по
  ...
  публичным запуском.
- ✅ **Сторожа починены** (commit `cd8fd20`): UX-аудит больше не плодит пустые агенды
  (cron задавал день месяца И день недели — cron считает их через «ИЛИ»; каденция и
  отказ писать новую агенду поверх незаполненной переехали в `ux-audit-review.mjs`);
  weekly review снова видит задачи (считает и чек-листы, не только таблицы);
  машинный поток сторожей вынесен из `tasks/backlog.md` (2162 строки → 265) в
  `tasks/docs-watch-archive.md`; help-watch заменяет своё прошлое напоминание.
  Удалены 42 незаполненные агенды `docs/ux-audit/ux-audit-*.md`.
- ✅ **Статусы задач сверены с кодом** — закрыты Code-exec, переменные «Старта»,
  фазы 0–4 аккаунт-дашборда; уточнены остатки по Фазе 4 и Batch B.
- ✅ **Перемерен вес сборки** — `tasks/perf-audit-2026-08-18.md`.
...
- [ ] Фаза 5 — гость vs вошедший: что видит гость, мягкий призыв войти ради синхронизации.
...
- [~] **Фаза 4 — B-3**: edge-функция трактует ATTACH как способности агента.
  Реально работают: веб-поиск, память, файлы, vision, code-exec, цитаты, MCP.
  Остаток — `tool-computer`: узел есть в палитре, но движок его не исполняет
  (проверено 2026-08-18 по `supabase/functions/builder-execute/index.ts`).
