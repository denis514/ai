...
⚠️ **Деплой-долг:** `supabase functions deploy account-delete` — до этого кнопка
«Удалить аккаунт» в проде показывает ошибку (данные не трогает).

Осталось из той же ревизии (open):
- [ ] Перенос схем гостя в облако при входе (`migrateLocalToCloud` написан,
  не подключён) + убрать текст «сохранение требует аккаунта» в кабинете.
- [ ] Журнал активности (серия дней): слияние гость→облако; `startedAt` курсов
  в облако.
...
- [ ] Замечания судьи: `hydrated` при полном падении сети; `skipDirtyRef` на
  пустом холсте; задокументировать компромисс «невольный выход → вход другого».
