...
## Builder — провайдеры моделей / локальный ИИ (обсуждено 2026-06-03)

- **«Local / Privacy mode (beta)»** — выполнять агент-узлы на локальной модели
  пользователя (Ollama, OpenAI-совместимый API).
  - Ограничение: облачный сервер НЕ видит `localhost` пользователя → возможно только
    в браузерном режиме (ручной запуск при открытом браузере) ИЛИ через туннель
    (публичный адрес, небезопасно). Расписание/доставка остаются серверными.
  - Подводные камни: серверные инструменты Claude (web_search, MCP, citations) на
    локальной модели не работают; качество следования инструкциям ниже; второй движок
    исполнения в поддержке.
  - Ценность: приватность (данные не уходят), нулевая стоимость токенов, оффлайн.
    Дифференциатор для сегмента «privacy-first».
  - **Подготовительный шаг:** абстракция «провайдер модели» (Claude / OpenAI-совместимый
    / Ollama), чтобы добавление локальной модели было настройкой, а не переписыванием.
- **Поднятие tier Anthropic** (Add funds) — снимает потолок 30k входных токенов/мин,
  тогда работает тяжёлый веб (открытие страниц web_fetch), не только поиск.
- Заметка: подписка Claude Pro/Max ($200/мес) НЕ применима как оплата API сервиса —
  только токены/BYOK.
