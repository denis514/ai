...
async function callClaude(apiKey: string, system: string, userContent: string, maxTokens: number, images: ImageInput[] = [], mcpServers: McpServer[] = [], webTools = false, codeExec = false, citationDocs: CitationDoc[] = [], onRateWait?: (sec: number) => Promise<void>) {
  ...
  if (webTools) {
    ...
    // max_uses 8 (было 3): Claude 5 запускает несколько поисков ПАРАЛЛЕЛЬНО,
    // и потолок 3 срабатывал с первого же хода (max_uses_exceeded), после чего
    // модель крутилась в повторах и сжигала ~50k токенов (живой прогон 2026-08-23).
