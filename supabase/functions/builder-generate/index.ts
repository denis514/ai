...
Что конструктор УМЕЕТ (не отказывай из-за этого):
- запуск по расписанию («каждое утро», «раз в неделю» — настраивается после
  сборки, в схему его не клади);
- текст от человека: «мой список дел», «отзыв, который я вставлю», «письмо,
  которое я скопирую» — это НОРМАЛЬНЫЙ вход через блок Старт, не отказ;
- доставка: output-telegram, output-email, output-calendar (создать событие
  в Google Calendar — это готовый выход, не вебхук);
- поиск в интернете (tool-search), вставленный текстовый файл (tool-file),
  картинка (tool-vision), выполнение кода (tool-code-exec).
...
Отказывай ТОЛЬКО если без невозможного пункта задача теряет смысл
(например, «читай мою почту» — без чтения почты делать нечего).
Если СУТЬ задачи держится на таком пункте — схему не собирай,
...
отзыва в Старт)"}.
Публикация в соцсети, сохранение в Notion/таблицу, картинка к посту — ВСЕГДА
довесок, а не суть: человеку полезны сами идеи/тексты. Пример: «раз в неделю
собирай идеи постов и публикуй в Instagram» → схема Старт → агент-контент
(с поиском) → вывод текста, note: «Публиковать в Instagram схема не будет —
идеи придут текстом, опубликуйте вручную». Если невозможный пункт — такой
довесок, задача остаётся полезной без него:
собери схему, а этот шаг замени выводом текста и добавь поле "note": одно
предложение на ${LANG[locale] || LANG.ru} языке, чего схема делать не будет
и что сделать вручную.`;
}
...
Deno.serve(async (req) => {
  ...
  try {
    for (let attempt = 0; attempt < 2 && !scheme; attempt++) {
      ...
      // Модель часто отказывает из-за довеска («…и опубликуй»), хотя сама же
      // предлагает «соберу идеи текстом». Поэтому первый отказ — переспрос:
      // «собери выполнимую часть + note». Второй отказ подряд — настоящий.
      if (parsed && (parsed as { error?: string }).error === 'unsupported') {
        const why = String((parsed as { why?: string }).why || '').slice(0, 300);
        if (attempt === 0) {
          lastWhy = `ты отказал («${why}»). Если задача полезна и без невозможного шага — ` +
            'собери схему для выполнимой части (невозможный шаг замени выводом текста) ' +
            'и добавь "note", чего схема не сделает. Если без этого шага задача бессмысленна — ' +
            'верни {"error":"unsupported","why":...} ещё раз';
          continue;
        }
