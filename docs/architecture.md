...
## 2026-08-22 — Vercel Web Analytics (@vercel/analytics)

Новая runtime-зависимость `@vercel/analytics` (~1-2 KB): счётчик посещений
Vercel, включён на панели проекта. Подключение — `inject()` в `src/main.jsx`
(мы на Vite, поэтому вариант `@vercel/analytics/next` из подсказок Vercel не
подходит). Без cookies и идентификации людей — согласия не требует, плашка
cookies его не касается; собирает агрегированные просмотры страниц и страну.
SPA-переходы отслеживаются автоматически (патч history.pushState). Скрипт
