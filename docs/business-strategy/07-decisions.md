...
## DECISION (2026-06-11): Collaborative Canvas (real-time co-editing) — ОТЛОЖЕНО

**Решение: ❌ не делать сейчас → ⚠️ пересмотреть позже (условно).**

**Контекст:** рассматривалось добавление совместной работы на Canvas в реальном времени
(как Figma/Miro) — несколько пользователей одновременно на одной схеме/карте. Проведён
независимый разбор в ролях PM / SaaS-архитектор / UX / CTO / инвестор.

**Почему отложено (на текущей стадии):**
- Платного тарифа ещё нет (paywall = Phase 2-3, не сделан); платящих пользователей — 0.
  Real-time co-editing монетизируется только Team-tier — для аудитории, которой пока нет.
- ICP покупает плейбук/операционные модели для себя (single-player), а не «доску для
  команды». Agent-builder и knowledge-map — артефакты одного владельца (прямой аналог —
  n8n, doc 09: вырос БЕЗ co-editing).
- Real-time multiplayer — самый дорогой и хрупкий пласт SaaS (CRDT, конфликты, presence,
  права, история). Для соло-founder'а — неподъёмный налог на поддержку + отвлечение от
  активации и первого доллара.
- Прямо противоречит PIVOT-PRODUCT-FIRST (фокус на single-player ценности).

**80/20 вместо real-time** (раньше, дёшево, вирусность без инфраструктуры реального
времени): (1) публичная share-ссылка view-only; (2) «клонировать Canvas себе»;
(3) асинхронные комментарии. История версий — следом.

**Технологии (если делать позже):** Liveblocks — быстрее всех, но платить до первого $1
абсурдно; Yjs — «правильно», но операционно тяжело соло; Supabase Realtime — закрывает
presence/комментарии, но НЕ co-editing графа (pub/sub, не CRDT); своя реализация — нет.

**Условие пересмотра (⚠️ → ✅):** все три сразу — (1) есть платящие одиночные пользователи
(доказан Pro); (2) ≥5 из них САМИ просят командный режим; (3) Team-tier валидирован
интервью по протоколу Phase 0 (§15a).

**Приоритет вместо:** paywall + первый платный тариф → активация single-player Builder →
share-ссылка/clone → комментарии → история версий → шаблоны/галерея → retention.

**Owner:** Denis · **Status:** DEFERRED (P4)

---

## DECISION (2026-08-19): слово «workflow» в интерфейсе остаётся

**Решение:** курсы в интерфейсе называются «workflow», как и было. Не заменяем
на «курс», «урок», «обучение» и синонимы.

**Откуда возник вопрос:** первый заполненный UX-аудит
(`docs/ux-audit/ux-audit-2026-08-18.md`) отметил «workflow» как жаргон для
новичка: курсы написаны простым языком, а обрамление — «WORKFLOW», «Завершить
workflow», «Workflow завершён!». Находка верная с точки зрения понятности.

**Почему всё равно оставляем:** это не случайный жаргон, а позиционирование.
`docs/strategy/01-positioning.md` прямо предписывает лейблы «workflow / playbook
/ system» вместо «обучение» (§ «Какие лейблы?»), а `tasks/roadmap.md` держит
анти-цель «возврат к LMS / Academy / Course Platform». Слово «курс» тянет за
собой ожидание «ты узнаешь», а продукт обещает «ты изменишь работу».
Понятность здесь проигрывает позиционированию сознательно.

**Что делаем вместо:** там, где «workflow» не несёт позиционной нагрузки —
пишем по-человечески. Пример: в тексте знакомства использована формулировка
«пошаговые разборы», она не нарушает ни стратегию, ни правило простого языка.

**Условие пересмотра:** если пользователи в интервью/поддержке будут
спотыкаться именно об это слово (а не о содержание) — вернуться и подобрать
третье слово, которое и не «курс», и понятно новичку.

**Owner:** Denis · **Status:** SUPERSEDED — отменено решением 2026-08-22 (см. ниже)


---

## DECISION (2026-08-22): простой язык побеждает — слово «workflow» уходит из интерфейса

**Решение:** интерфейс говорит простыми словами. Разборы Atlas называются
«разбор» (en «guide», fi «opas»), схемы конструктора — «схема»
(en «automation», fi «automaatio»). Слово «workflow» из видимых текстов
интерфейса убрано во всех трёх языках.

**Что отменяется:** DECISION (2026-08-19) «слово workflow остаётся» и цель
P0 №1 бэклога в старой формулировке («0 строк курс/обучение, workflow-словарь»).
Предписание лейблов «workflow / playbook / system» в
`docs/strategy/01-positioning.md` § «Какие лейблы?» считать исправленным этим
решением: позиционирование «не LMS» сохраняется выбором слова — «разбор»
обещает изменение работы, а не «прослушанный курс», при этом понятен новичку.

**Почему:** владелец выбрал понятность (2026-08-22, явное решение при полном
знании конфликта с решением 19.08). Правило `docs/plain-language-ui.md`
(ОБЯЗАТЕЛЬНО) и UX-аудит 2026-08-18 указывали в ту же сторону.

**Границы:** термин «workflow» остаётся в СОДЕРЖИМОМ (узлы Atlas об AI
workflows как о концепте, тексты разборов, id и ключи переводов, названия
файлов/маршрутов) — это предмет объяснения, а не интерфейсный ярлык.
Контентная волна (тексты разборов, узлы) — отдельная задача.

**Owner:** Denis · **Status:** ACCEPTED


---

## DECISION (2026-08-23): строка «опиши задачу» вместо «Старт» + спонсорские автосборки

**Решение основателя:**
1. Первый экран конструктора: вместо кнопки «Старт» — строка ввода с
   самопечатающимися примерами «по делам»; под ней кнопка «Выбрать из
   шаблонов» (тем же стилем) и малая ссылка «пустой холст».
2. «Текст → схема» работает строго из наших кубиков (валидация обязательна).
3. **Основатель спонсирует 3 автосборки в день** на гостя со своего отдельного
   ключа (workspace с месячным потолком; ключ в секретах сервера,
   `ATLAS_SPONSOR_KEY`). У пользователей со своим ключом — на их ключе, без
   лимита 3/день. Защита: лимит на браузер/день + на IP + месячный потолок.
4. **Формулировка лимита — критична** (уточнение основателя): лимитируются
   только АВТОсборки; сборка руками безлимитна, и тексты обязаны это
   проговаривать («Осталось 2 автосборки на сегодня · руками — без
   ограничений»). Нельзя дать прочитать «3 бесплатных сборки вообще».

**Контекст:** doc 11 (n8n text-to-workflow как главный крючок активации;
Anthropic закрыл «расписание» как отличие — наш питч: доставка + простота).
**Owner:** Denis · **Status:** ACCEPTED
