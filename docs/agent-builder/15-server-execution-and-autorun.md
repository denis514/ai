...
- Лимит времени одного вызова модели: 100 с (`BUILDER_CLAUDE_TIMEOUT_MS`); шаг, не уложившийся, закрывается ошибкой «не уложился во время», а не убивает функцию молча. «Жнец»: прогон со статусом `running` старше 4 минут помечается сбоем при следующем запросе и не блокирует новые запуски (2026-08-23, поймано живым прогоном). Веб-поиск — `web_search_20260209`.
- Модель: `claude-sonnet-5`, «размышление» выключено (`thinking: disabled`) — иначе оно расходует `max_tokens` и ломает узел «условие-агент» с лимитом 8 токенов (секрет `BUILDER_MODEL` переопределяет модель без передеплоя; до 2026-08-23 была `claude-sonnet-4-5`, вывод которой намечен не ранее 2026-09-29). Тариф вывода — поле `tier` (S/M/L → max_tokens).
