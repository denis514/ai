...
- **Стиль — ТОЛЬКО из стайлгайда (ОБЯЗАТЕЛЬНО).** Источник правды по визуалу —
  `/styleguide` (`src/styleguide/StyleGuide.jsx`). Общие элементы (кнопки, переключатели,
  чипы, FAB, переключатель языка `LanguageSwitcher` и т.д.) берутся как есть — их классы
  единственны и одинаковы на ВСЕХ поверхностях (Atlas, Builder, аккаунт, лендинг, админка).
  **Запрещено** локально переопределять стиль общего компонента под конкретную страницу
  (никаких `.builder-lang .lang-switcher__btn{…}` и подобного). Нет элемента в стайлгайде —
  сперва добавить его туда, потом использовать.
...
**Барьер мобильной версии (ОБЯЗАТЕЛЬНО после десктопной вёрстки):** для изменений
UI (`*.css`, `src/**/*.jsx`, `src/builder/**`) ПОСЛЕ того как сделан и проверен
десктоп — прогнать `skills/mobile-gate/` (адаптив + тач + a11y, со скриншотом на
390×844). Вердикт **BLOCK** = НЕ пушить, сперва починить мобайл. Логика/контент
без вёрстки — мобильный барьер не нужен.
