...
Claude **обязан** написать короткий итог по фиксированному формату.
...
💡 Почему:
[1-2 предложения. Зачем это нужно — какая проблема решается.]

⚙️ Как это теперь работает:
[2-4 предложения. Новая логика / поведение системы.]

🔒 На что это влияет:
[Перечисли затронутые слои: security / архитектура / UX / производительность /
API / backend / frontend / scalability / стоимость / cost / GDPR.]

⚠️ Важные замечания или риски:
[Конкретно. Если нет — «нет известных рисков».]
...
- Русский язык (см. § 14a)
- Plain language — как Product Owner объясняет founder'у, не overly technical
- Когда меняются Структура проекта / API / Git / Vercel / Security / AI architecture /
  Cost — выделять отдельной строкой в секции 🔒
- 🚨 BREAKING changes — обязательно в секции ⚠️ с явным указанием
...
**Подробная спецификация:** `skills/post-action-summary/SKILL.md`
