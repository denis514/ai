...
const SECTIONS = [
  ...
  },
  {
    ...
    items: [
      ...
      ['builder.help.run_3', 'Расписание конкретной схемы — часики рядом с «Запуск» (работает на сервере, даже когда сайт закрыт). Все автозапуски и история прогонов — в «Консоли» → вкладка «Автозапуски» (там же можно очистить историю).'],
      ...
      ['builder.help.run_5', 'Вебхук — секретная ссылка (там же, под часиками): запрос на неё запускает схему извне, а данные запроса становятся задачей. Удобно для «пришло событие → обработать».'],
    ],
  },
  {
    id: 'keys',
