...
const SECTIONS = [
  {
    ...
    items: [
      ...
      ['builder.help.start_2', 'Минимальная схема: «User Input» → «Агент» → «Выход» (Markdown, Telegram, Email или Календарь).'],
      ['builder.help.start_3', 'Задачу пишут внутри узла «User Input»: кликните по нему — справа откроется поле задачи. Отдельного поля снизу больше нет.'],
  ...
  },
  {
    ...
    items: [
      ...
      ['builder.help.grp_flow', 'Поток — User Input (старт) и выходы: Markdown (на экран), Telegram, Email, Google Календарь.'],
    ],
  },
  {
    id: 'variables',
    ...
    items: [
      ...
      ['builder.help.var_2', 'Где задавать: кликните узел «User Input» → блок «Переменные» → «Добавить переменную». Слева впишите имя (без скобок), справа — значение.'],
  ...
  },
  {
    ...
    items: [
      ...
      ['builder.help.run_2', 'Размер ответа (S / M / L) и примерную цену видно в узле «User Input». Перед запуском схема проверяется на несоединённые узлы и отсутствие агента.'],
  ...
  },
  {
    ...
    items: [
      ...
      ['builder.help.mis_4', '{{переменная}} написана, но не задана в «User Input» — останется в тексте как есть. Задайте её в блоке «Переменные».'],
    ],
  },
];
