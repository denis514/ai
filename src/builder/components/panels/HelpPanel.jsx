...
const SECTIONS = [
  {
    ...
    items: [
      ...
      ['builder.help.start_4', 'Схема сохраняется сама — кнопки «Сохранить» нет. Название схемы — по центру сверху, там же список «Мои схемы».'],
      ['builder.help.start_5', 'Не знаете, с чего начать: слева в узкой полоске есть кнопка «Тур», она проведёт по шагам от первой сборки до автозапуска.'],
    ],
  },
  {
    id: 'connect',
  ...
  },
  {
    ...
    items: [
      ...
      ['builder.help.grp_tools', 'Инструменты — способности агента, прикрепляются к нему сбоку. Работают: Веб-поиск, Файлы, Зрение, Память, Код, Цитаты, MCP-коннектор.'],
  ...
  },
  {
    id: 'tools',
    icon: 'plug',
    titleKey: 'builder.help.s_tools',
    titleFb: 'Инструменты: что умеют и как настроить',
    items: [
      ['builder.help.tool_search', 'Веб-поиск: агент ищет свежее в интернете. Чтобы он открыл конкретную страницу, вставьте её ссылку прямо в задачу «Старт». Узел прикрепляется к агенту сбоку.'],
      ['builder.help.tool_files', 'Файлы: чтобы агент прочитал документ или страницу, вставьте ссылку (URL) на неё в задачу. Загрузки файла с компьютера пока нет — работает по ссылке. Очень длинную страницу агент читает с начала, не целиком.'],
      ['builder.help.tool_vision', 'Зрение: чтобы агент посмотрел картинку, вставьте ссылку на изображение в задачу и прикрепите узел «Зрение» к агенту.'],
      ['builder.help.tool_memory', 'Память: помогает агенту держать одно и то же (тон, правила) в пределах запуска и между шагами цепочки. Просто прикрепите к агенту.'],
      ['builder.help.tool_mcp', 'MCP (свой сервис): сначала в разделе с замком сверху, вкладка «MCP», нажмите «Добавить» и впишите имя, ссылку на сервер и токен (если нужен). Потом прикрепите узел «MCP» к агенту и выберите сервер. Это удалённый сервер по ссылке, не локальный контейнер.'],
      ['builder.help.tool_advanced', 'Код и Цитаты уже работают: Код реально считает (Python), Цитаты подкрепляют ответ ссылками на источники. Управление компьютером убрано из набора, пока оно не выполняется по-настоящему.'],
    ],
  },
  {
    id: 'variables',
  ...
  },
  {
    ...
    items: [
      ...
      ['builder.help.run_2', 'Размер ответа (Коротко / Средне / Подробно) и примерное число токенов видно в узле «Старт». Перед запуском схема проверяется на несоединённые узлы и отсутствие агента.'],
      ['builder.help.run_6', 'Окно «Консоль» (кнопка с терминалом сверху) — одно окно с вкладками: «Запуск» (ход выполнения и результат), «Код» (схема как код) и «Автозапуски».'],
      ['builder.help.run_7', '«Консоль» → «Код»: показывает текущую схему кодом и наоборот — можно вставить готовый код и собрать узлы на холсте автоматически, с проверкой связей.'],
      ['builder.help.run_3', 'Расписание конкретной схемы: часики рядом с «Запуск» открывают панель справа. Вкладки «Однократно» и «Многократно» (каждые N минут, ежедневно, по числам месяца, раз в год), затем «Создать автозапуск». Работает на сервере, даже когда сайт закрыт. Ниже в той же панели: все автозапуски и история прогонов, они же в «Консоли» на вкладке «Автозапуски».'],
      ...
      ['builder.help.run_5', 'Вебхук: секретная ссылка в той же панели, под расписанием. Запрос на неё запускает схему извне, а данные запроса становятся задачей. Удобно, когда пришло событие и его надо обработать.'],
    ],
  },
  {
    id: 'keys',
