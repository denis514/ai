  ...
  return (
    <div className="sg" data-sg-theme={theme}>
      ...
      <main className="sg-main">
        ...
        <Section id="header-standard" title="Стандарт шапки (ОБЯЗАТЕЛЬНО)"
          note="Единый стандарт для ВСЕХ поверхностей (Atlas, Builder, кабинет, лендинг, админка): СЛЕВА одной группой: кнопка «назад/контекст» (плашка поверхности), СРАЗУ ЗА НЕЙ — <ThemeSwitcher /> и затем <LanguageSwitcher />, именно в этом порядке (как на карте Atlas), всегда общими компонентами. Никаких локальных реализаций темы/языка в шапках. Дублировать переключатели в настройках страниц не нужно — они всегда доступны из шапки.">
          <div className="sg-langrow">
            <ThemeSwitcher title="Тема" />
            <LanguageSwitcher title="Язык" />
            <span className="sg-langrow__hint">Правый кластер шапки: тема, затем язык.</span>
          </div>
        </Section>

        <Section id="theme-switcher" title="Переключатель темы"
          note="Единый компонент <ThemeSwitcher>, парный к LanguageSwitcher: та же круглая кнопка и попап (классы .lang-switcher__* переиспользуются как единственный источник стиля — своих стилей у компонента нет). Иконка кнопки = текущий режим (солнце/луна/ноутбук). Выпадашка — КОМПАКТНАЯ: столбик из трёх иконок шириной с кнопку, активный режим отмечен кружком-плашкой; подписи только в title/aria. Все попапы шапки взаимоисключаются через popoverBus (открыт всегда один). Использовать ВЕЗДЕ, где нужна смена темы; используется в шапках всех поверхностей (см. «Стандарт шапки»).">
          <div className="sg-langrow">
            <ThemeSwitcher title="Тема" />
